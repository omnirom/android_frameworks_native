...
struct BufferPendingRelease final {
    using AsyncConnector = core::AsyncFunctionStd<void(BufferPendingRelease)>;
    ...
    friend auto operator==(const BufferPendingRelease&, const BufferPendingRelease&)
            -> bool = default;
};

inline auto toString(const BufferPendingRelease& event) -> std::string {
...
inline auto format_as(const BufferPendingRelease& event) -> std::string {
